...
function CategoryManagement() {
  ...
  return <section className="admin-page"><div className="page-title"><span>Catalog control</span><h2>Categories</h2><p>Create product categories once and use them throughout the catalog.</p></div>{message && <div className="admin-alert">{message}</div>}
    ...
    <section className="admin-panel"><div className="panel-heading"><div><span>Available categories</span><h3>{categories.length} Categories</h3></div><label className="search-field category-search"><Search size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search categories" /></label></div><div className="category-list">{visibleCategories.map((category) => <article className="category-row" key={category._id}><div><strong>{category.name}</strong><small>{category.productCount} product{category.productCount === 1 ? '' : 's'}{category.isSystem ? ' · Existing category' : ''}</small></div><div className="row-actions"><button onClick={() => startEdit(category)} aria-label={`Rename ${category.name}`}><Edit size={17} /></button><button className="danger-button" onClick={() => remove(category)} disabled={category.isSystem} aria-label={`Delete ${category.name}`} title={category.isSystem ? 'Existing categories are protected' : 'Delete category'}><Trash2 size={17} /></button></div></article>)}</div>{!visibleCategories.length && <div className="empty-state"><h3>No categories found</h3><p>Add a category or adjust your search.</p></div>}</section></section>
}
