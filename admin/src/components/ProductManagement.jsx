...
function ProductManagement() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setMessage(requestErrorMessage(error, 'Unable to load products.'))
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSave = async (event) => {
    ...
    try {
      ...
      setMessage(requestErrorMessage(error, 'Unable to save product.'))
    } finally {
      setSaving(false)
    }
  }
  ...
  const saveQuickEdit = async (product, form) => {
    ...
    try {
      ...
      await fetchData()
      setMessage(`${product.nameEnglish} updated successfully.`)
    } catch (error) {
      setMessage(requestErrorMessage(error, 'Unable to update product.'))
    } finally {
      setQuickSaving('')
    }
  }

  const saveImageChange = async (product, file, remove = false) => {
    if (!product || (!file && !remove)) return
    setImageUploading(product._id)
    setMessage('')
    try {
      const formData = new FormData()
      formData.append('productIndex', product.productIndex)
      formData.append('nameTamil', product.nameTamil)
      formData.append('nameEnglish', product.nameEnglish)
      formData.append('category', product.category || 'all')
      formData.append('unit', product.unit || 'kg')
      ...
      await axios.put(`${API_URL}/api/products/${product._id}`, formData)
      await fetchData()
      setMessage(`${product.nameEnglish} image ${remove ? 'removed' : 'updated'} successfully.`)
    } catch (error) {
      setMessage(requestErrorMessage(error, 'Unable to update product image.'))
    } finally {
      setImageUploading('')
    }
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`${API_URL}/api/products/${deleteTarget._id}`)
      await fetchData()
      setMessage(`${deleteTarget.nameEnglish} was deleted successfully.`)
    } catch (error) {
      setMessage(requestErrorMessage(error, 'Unable to delete product.'))
    } finally {
      setDeleting(null)
  ...
  return <section className="admin-page">
    ...
    {showForm && <form className="admin-panel product-form" onSubmit={handleSave}>
      <div className="panel-heading">
        <div><span>{editingProduct ? 'Update product' : 'New product'}</span><h3>{editingProduct ? 'Edit Product' : 'Add Product'}</h3></div>
        <button type="button" className="icon-button" onClick={resetForm} aria-label="Close product form"><X /></button>
      </div>
      <div className="form-section">
        <h4>Product information</h4>
        <div className="form-grid">
          <label>Product Index<input name="productIndex" pattern="PB-\d{3,}" title="Use the format PB-001" defaultValue={editingProduct?.productIndex || ''} required /></label>
          <label>Tamil Name<input name="nameTamil" defaultValue={editingProduct?.nameTamil} required /></label>
          <label>English Name<input name="nameEnglish" defaultValue={editingProduct?.nameEnglish} required /></label>
          <label>Category<select name="category" defaultValue={editingProduct?.category || 'all'}><option value="all">All products</option>{categories.map((item) => <option value={item.slug} key={item._id}>{item.name}</option>)}</select></label>
        </div>
      ...
      <div className="form-section">
        <h4>Product image</h4>
        <div className="image-input"><input name="image" type="file" accept="image/*" onChange={handleImage} /><button type="button" onClick={() => { setImagePreview(null); setRemoveImage(true) }}>Remove image</button></div>
        {removeImage && <p className="form-help">This image will be removed when you save the product.</p>}
        {imagePreview && <img className="image-preview" src={imagePreview} alt="Selected product preview" />}
      </div>
      <div className="form-actions"><button className="primary-action" disabled={saving}><Save size={17} />{saving ? 'Saving…' : editingProduct ? 'Save Changes' : 'Save Product'}</button><button type="button" className="secondary-action" onClick={resetForm}>Cancel</button></div>
    </form>}
    ...
    <section className="admin-panel">
      ...
      {loading ? <div className="loading-grid">{Array.from({ length: 5 }).map((_, index) => <div className="skeleton" key={index} />)}</div> : visibleProducts.length ? <div className="product-list">
        {visibleProducts.map((product) => <article className="admin-product-card" key={product._id}>
          <div className="product-image-controls">
            {product.imageUrl ? <img className="product-image-thumbnail" src={product.imageUrl} alt={product.nameEnglish} /> : <div className="product-image-placeholder" aria-label="PB image placeholder">PB</div>}
            <label className="inline-image-control">
              <input type="file" accept="image/*" disabled={imageUploading === product._id} onChange={(event) => { const file = event.target.files[0]; if (file) saveImageChange(product, file) }} />
              <ImagePlus size={15} />{imageUploading === product._id ? 'Saving…' : product.imageUrl ? 'Change Image' : 'Add Image'}
            </label>
            {product.imageUrl && <button type="button" className="inline-image-remove" disabled={imageUploading === product._id} onClick={() => saveImageChange(product, null, true)}>Remove Image</button>}
          </div>
          ...
          <span className={`stock-status stock-${stockStatusFor(product)}`}>{stockStatusFor(product) === 'out-of-stock' ? 'No Stock' : stockStatusFor(product) === 'low-stock' ? 'Low Stock' : 'In Stock'}</span>
          <div className="row-actions"><button type="button" onClick={() => startEdit(product)} aria-label={`Edit ${product.nameEnglish}`}><Edit size={17} /></button><button type="button" className="danger-button" onClick={() => setDeleteTarget(product)} aria-label={`Delete ${product.nameEnglish}`}><Trash2 size={17} /></button></div>
        </article>)}
      </div> : <div className="empty-state"><h3>No products found</h3><p>Adjust the filters or add your first product.</p><button className="primary-action" onClick={() => { resetForm(); setShowForm(true) }}><Plus size={17} />Add Product</button></div>}
    </section>

    {deleteTarget && <div className="confirm-backdrop" role="dialog" aria-modal="true" aria-labelledby="delete-title"><div className="confirm-dialog"><h3 id="delete-title">Delete product?</h3><p>Are you sure you want to delete <strong>{deleteTarget.nameEnglish}</strong>? This cannot be undone.</p><div className="form-actions"><button className="danger-action" onClick={handleDelete} disabled={Boolean(deleting)}>{deleting ? 'Deleting…' : 'Delete Product'}</button><button className="secondary-action" onClick={() => setDeleteTarget(null)}>Cancel</button></div></div></div>}
  </section>
}
